...
import {Observable, merge, of, timer, Subject} from 'rxjs';
import {switchMap, shareReplay, map, catchError, takeUntil} from 'rxjs/operators';
...
export class QuotaService implements OnDestroy {
  ...
  createQuota(payload: Quota): Observable<Record<string, never>> {
    return this._http.post<Record<string, never>>(this._baseUrl, payload).pipe(catchError(_ => of({})));
  }

  updateQuota(quotaName: string, payload: QuotaVariables): Observable<Record<string, never>> {
    return this._http
      .patch<Record<string, never>>(this._baseUrl + '/' + quotaName, payload)
